    ...
    try {
        ...
        for (let i = 0; i < incomingSteps.length; i++) {
            ...
            for (const file of targetedFiles) {
                ...
                if (stepUploadError) { throw stepUploadError; }
...
async function fetchFreeStepImage(query: string): Promise<string | null> {
    try {
        ...
        const response = await fetch(
            `https://api.unsplash.com/search/photos?page=1&per_page=1&query=${encodeURIComponent(query)}&orientation=landscape`,
            {
                headers: {
...
export const generateAiRecipe = async (req: Request, res: Response) => {
    try {
        ...
        const stepsWithImages = await Promise.all(
            rawJson.steps.map(async (step: any, index: number) => {
